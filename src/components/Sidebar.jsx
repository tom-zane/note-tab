  ...
  return (
    <>
      <div 
        ...
        } h-screen bg-[var(--bg-primary)] flex flex-col transition-all duration-300`}
      >
            ...
            <div
              ...
              className={`p-3 cursor-pointer flex justify-between items-center ${
                activeNoteId === note.id
                  ? 'bg-[var(--text-accent)] bg-opacity-20'
                  : 'hover:bg-[var(--bg-secondary)] hover:bg-opacity-50'
              }`}
            >
              {isCollapsed ? (
                ...
                <>
                  <h3 
                    className={`font-mono  ${activeNoteId === note.id ? 'text-[var(--bg-primary)]' : 'text-[var(--text-primary)]'}  truncate`}
                    style={{ fontSize: `${settings.sidebarSize}px` }}
                  >
                    {note.title || 'Untitled Note'}
                  </h3>
                  <button
                    onClick={(e) => handleDeleteClick(e, note)}
                    className="text-[var(--text-secondary)] hover:text-[var(--button-danger)] p-1 rounded"
                  >
                    <FiTrash2 size={14} />
                  </button>
                </>
        ...
        <button
          ...
          className={`m-4 p-2 bg-[var(--button-secondary)] text-[var(--text-primary)] rounded-lg flex items-center justify-center hover:bg-opacity-80 transition-colors text-sm ${
            isCollapsed ? 'px-2' : ''
          }`}
        >
